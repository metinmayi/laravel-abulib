    ...
    <form id="literatureForm" action="/literature" method="POST" enctype="multipart/form-data">
      ...
      <div id="languages">
        ...
        <div class="language-form mb-8 p-6 border rounded-lg bg-white shadow-md">
          <!-- Collapsible Header -->
          <button type="button" onclick="toggleSection('{{ $val }}')" class="text-xl font-semibold text-gray-700 w-full text-left mb-4">
          ...
          <div id="{{ $val }}-section" class="hidden">
            ...
            <!-- Language Title -->
            <label for="{{$val . '-title'}}" class="block text-sm font-medium mb-2">{{ ucfirst($val) . ' Title' }}:</label>
            <input type="text" id="{{$val . '-title'}}" name="literatures[{{$val}}][title]" placeholder="{{'Enter the ' . $val .' title of the literature'}}" required class="block w-full p-2 border rounded mb-4">

            <!-- Language Description -->
            <label for="{{$val . '-description'}}" class="block text-sm font-medium mb-2">{{ ucfirst($val) . ' Description' }}:</label>
            <textarea id="{{$val . '-description'}}" name="literatures[{{$val}}][description]" placeholder="{{'Enter the ' . $val .' description of the literature'}}" class="block w-full p-2 border rounded mb-4"></textarea>

            <!-- Language File Upload (Optional for each language) -->
            <label for="{{$val . '-file'}}" class="block text-sm font-medium mb-2">{{ ucfirst($val) . ' File' }} (Optional):</label>
            <input type="file" id="{{$val . '-file'}}" name="literatures[{{$val}}][file]" accept="application/pdf" class="block w-full p-2 border rounded mb-4">
