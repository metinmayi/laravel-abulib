        ...
        <div class="w-3/4">
                ...
                <div class="flex flex-wrap gap-4">
                    @foreach ($literatureItem['availableVariants'] as $variant)
                        <a href="{{ route('read.index', ['variantId' => $variant['id']]) }}"
                            class="text-orange-600 border border-orange-600 px-2 py-1 rounded">{{ __('messages.' . $variant['language']) }}</a>
                    @endforeach
                </div>
                <h3 class="text-orange-400 text-3xl mt-8">{{ __('messages.' . $literatureItem['category']) }}
                </h3>
            ...
            @if (isset($literatureItem['url']))
                <a href="{{ route('read.getLiteratureFile', ['id' => $literatureItem['id']]) }}"
                    class="text-white bg-green-600 border border-green-600 px-6 py-3 rounded">{{ __('Read') }}</a>
            @else
